...
fetch("http://localhost:3000/api/novel?status=true")
    ...
    .then(data => {
        ...
        loadFullItem(data);
        loadNewestNovel(data.sort((a, b) => <any>new Date(b.last_update) - <any>new Date(a.last_update)));
    });

function loadHotItem(data: any[]) {
...
function loadFullItem(data: any[]) {
...
function headerTypeListHome(data: any[]) {
...
function headerListHome(data: any[]) {
...
function formatTimeDifferenceHome(timestamp: number) {
...
async function loadNewestNovel(data: any[]) {
...
document.querySelector(".header__form-search")!.addEventListener("submit", (event) => {
    ...
    if ((document.querySelector(".search-story") as HTMLInputElement).value.length > 0) {
        searchHandleHome((document.querySelector(".search-story") as HTMLInputElement).value);
    }
});
