...
const loadTitleNovel = (title) => {
    document.querySelector("#novel-title").textContent = title;
};
const loadTitleChapter = (title, chapterNumber) => {
...
const headerTypeListChapter = (data) => {
...
const changeChapterHandle = (option) => {
    if (option === "next") {
...
document.querySelector(".header__form-search").addEventListener("submit", (event) => {
    ...
    if (document.querySelector(".search-story").value.length > 0) {
        searchHandleChapter(document.querySelector(".search-story").value);
    }
});
