fetch("http://localhost:3000/api/novel")
    .then(res => res.json())
    .then(data => loadNovelTable(data));
const loadNovelTable = (data) => {
    const element = document.querySelector("#novel-table");
    let html = "";
    data.forEach((item, index) => {
        html += `<tr>
        <th scope="row">${index + 1}</th>
        <th scope="row" class="table-scope">
            <span>${item._id}</span>
        </th>
        <th scope="row">
            <img class="table-img" src="../../../assets/images/${item.image}"
                alt="">
        </th>
        <td class="table-scope">
            <span>${item.name}</span>
        </td>
        <td id="chapter-number-${index}"></td>
        <td class="table-scope">
            <span id="hot-list-${index}"></span>
        </td>
        <td class="table-scope">
            <span id="type-list-${index}"></span>
        </td>`;
        if (item.status === true) {
            html += `<td>
            <span
        ...
        html += `<td class="table-scope">
            ...
            <td>
                ...
                <button onclick="deleteNovel('${item._id}')" class="option-btn">
        ...
        fetch("http://localhost:3000/api/chapter?novel_id=" + item._id)
            .then(res => res.json())
            .then(data => {
            // console.log(data);
            loadLabel(data.length, `#chapter-number-${index}`);
        });
        item.category_id.forEach((item) => {
            fetch("http://localhost:3000/api/category?_id=" + item)
                .then(res => res.json())
                .then(data => {
                // console.log(data);
                if (data[0].type === 0) {
                    addLabel(data[0].title, `#hot-list-${index}`);
                }
                else {
                    addLabel(data[0].title, `#type-list-${index}`);
                }
            });
        });
        // console.log(item.category_id.join(","));
    });
    element.innerHTML = html;
};
const loadLabel = (title, element) => {
    document.querySelector(element).textContent = title;
};
const addLabel = (title, element) => {
    if (document.querySelector(element).textContent.length > 0) {
